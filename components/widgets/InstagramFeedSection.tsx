...
function ReelCard({ post, wide }: { post: InstagramPost; wide?: boolean }) {
  ...
  if (playing && shortcode) {
    return (
      <div className={`${w} rounded-2xl overflow-hidden relative bg-black shrink-0`}
  ...
  return (
    <button
      ...
      className={`${w} rounded-2xl overflow-hidden relative flex flex-col shrink-0`}
      style={{
        background: "linear-gradient(160deg,#833AB4,#FD1D1D,#F77737)",
        aspectRatio: wide ? "9/16" : undefined,
        height: wide ? undefined : "256px",
      }}
    >
      {post.thumbnailUrl && (
        <img
      ...
      <div className="absolute bottom-0 left-0 right-0 p-2.5">
        ...
        {(post.likeCount > 0 || (post.viewCount != null && post.viewCount > 0)) && (
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {post.viewCount != null && post.viewCount > 0 && (
              <span className="text-[10px] text-white/70">👁 {formatCount(post.viewCount)}</span>
            )}
          </div>
        )}
      </div>
...
function FeedCard({ post }: { post: InstagramPost }) {
  ...
  if (open && shortcode) {
    return (
      <div className="rounded-xl overflow-hidden bg-black relative" style={{ minHeight: 420 }}>
        <iframe
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-square bg-gradient-to-br from-gray-100 to-gray-200">
        ...
        {/* 탭 힌트 오버레이 */}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 active:opacity-100 bg-black/20 transition-opacity">
