...
async function addressDocument(address) {
  ...
  const response = await fetch(url, { headers: { Authorization: `KakaoAK ${KAKAO_KEY}` } });
...
async function buildingTitles(addressRow) {
  const address = addressRow.address ?? {};
  const bCode = String(address.b_code ?? "");
  if (bCode.length < 10) return [];
  const url = new URL("https://apis.data.go.kr/1613000/BldRgstHubService/getBrTitleInfo");
  url.searchParams.set("serviceKey", DATA_KEY); url.searchParams.set("sigunguCd", bCode.slice(0, 5)); url.searchParams.set("bjdongCd", bCode.slice(5, 10));
  ...
  const response = await fetch(url); if (!response.ok) throw new Error(`Building HUB ${response.status}`);
  const data = await response.json(); const items = data.response?.body?.items?.item ?? []; return Array.isArray(items) ? items : [items];
}
function normalized(value) { return String(value ?? "").replace(/\s+/g, "").toLowerCase(); }
function chooseTitle(building, titles) {
  const exactAddress = titles.filter(item => normalized(item.newPlatPlc || item.platPlc).includes(normalized(building.address.replace(/^인천(광역시)?\s*/, ""))));
  const pool = exactAddress.length ? exactAddress : titles;
  return pool.sort((a, b) => (normalized(b.bldNm).includes(normalized(building.name)) ? 3 : 0) - (normalized(a.bldNm).includes(normalized(building.name)) ? 3 : 0))[0] ?? null;
}
...
for (const building of (LIMIT ? buildings.slice(0, LIMIT) : buildings)) {
  ...
  try {
    ...
    const title = chooseTitle(building, await buildingTitles(addressRow)); if (!title) { report.push({ id: building.id, status: "register_not_found" }); continue; }
...
console.log(JSON.stringify({ mode: APPLY ? "apply" : "dry-run", checked: report.length, verified: report.filter(item => /verified|updated/.test(item.status)).length, report }, null, 2));
