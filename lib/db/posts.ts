...
function rowToPost(row: Record<string, unknown>): Post {
  return {
    ...
    authorAvatarUrl: (row.author_avatar_url as string | null | undefined) ?? undefined,
...
export async function updatePost(
  ...
  try {
    ...
    if (input.images   !== undefined) patch.image_urls  = input.images;
    if (input.videos   !== undefined) patch.video_urls  = input.videos;
    const { data, error } = await supabase
      .from('community_posts')
      .update(patch)
      .eq('id', id)
