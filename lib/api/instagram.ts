...
export interface InstagramPost {
  ...
  hashtags?: string[];
  postedAt: string;
  contentType?: "POST" | "REEL" | "STORY" | "CAROUSEL" | string;
  category?: string;
  displayName?: string;
...
export interface FetchFeedsParams {
  ...
  tag?: string;
  category?: string;
  contentType?: "ALL" | "POST" | "REEL" | "STORY" | "CAROUSEL";
}
...
function normalize(raw: any): InstagramPost | null {
  ...
  return {
    ...
    contentType: raw.contentType ?? raw.content_type ?? (raw.is_story ? "STORY" : isReel ? "REEL" : mediaType) ?? undefined,
    category: raw.category ?? "지역소식",
    displayName: raw.displayName ?? raw.display_name ?? raw.account_name ?? undefined,
    profileImageUrl: raw.profileImageUrl ?? raw.profile_image_url ?? undefined,
    followerCount: Number(raw.followerCount ?? raw.follower_count ?? 0),
    isStory: Boolean(raw.isStory ?? raw.is_story ?? [raw.content_type, raw.media_type].some(value => String(value ?? "").toUpperCase() === "STORY")),
    expiresAt: raw.expiresAt ?? raw.expires_at ?? undefined,
    relevanceScore: Number(raw.relevanceScore ?? raw.relevance_score ?? 0),
    collectedAt: raw.collectedAt ?? raw.collected_at ?? undefined,
  };
}
...
async function fetchFromSupabase(params: FetchFeedsParams): Promise<InstagramPost[]> {
  try {
    ...
    if (params.category && params.category !== "전체") q = q.eq("category", params.category);
    if (params.contentType && params.contentType !== "ALL") q = q.eq("content_type", params.contentType);

    const { data, error } = await q;
    if (error || !data?.length) return [];

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return data.map(normalize).filter((post: InstagramPost | null): post is InstagramPost => post !== null);
  } catch {
    return [];
  }
}
...
export async function fetchInstagramFeeds(params: FetchFeedsParams = {}): Promise<InstagramPost[]> {
  ...
  if (params.limit != null) localParams.set("limit", String(params.limit));
  if (params.page != null) localParams.set("page", String(params.page));
  if (params.sort) localParams.set("sort", params.sort);
  if (params.category) localParams.set("category", params.category);
  if (params.contentType) localParams.set("type", params.contentType);
  if (params.reel) localParams.set("type", "REEL");
  try {
    const response = await fetch(`/api/social-content?${localParams.toString()}`, {
