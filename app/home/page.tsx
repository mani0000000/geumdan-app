...
const quickMenus = [
  ...
  { icon: ShoppingBag,   label: "중고거래",href: "/community/?tab=커뮤니티&cat=중고거래",color: "#C2255C" },
  { icon: Star,          label: "즐겨찾기",href: "/mypage/#favorites",                   color: "#D9480F" },
];
    ...
    <>
      ...
      <section className="mx-4 mb-1 space-y-3">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-4 pt-4 pb-3 flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[18px] font-bold text-[#1d1d1f]">{multtae.name}</span>
            ...
            {nextLowTide && (
              <div className="bg-blue-50 rounded-xl px-3 py-2 text-center min-w-[78px]">
                ...
                <p className="text-[20px] font-black text-[#0071e3] leading-tight">{nextLowTide.timeStr}</p>
              </div>
            )}
          </div>
          ...
          {/* 조석 곡선 차트 */}
          {(() => {
            const W = 320; const H = 84; const PAD_X = 14; const PAD_Y = 10;
            ...
            return (
              <div className="border-t border-gray-50 px-1 pt-2 pb-1">
                <svg viewBox={`0 0 ${W} ${H + 28}`} className="w-full" style={{ height: 116 }}>
                  <defs>
                  ...
                  {pts.map((p, i) => {
                    ...
                    return (
                      <g key={i}>
                        <circle cx={p.x} cy={p.y} r={4} fill={isLow ? "#BFDBFE" : "#0071e3"} stroke="white" strokeWidth="1.5" />
                        <text x={p.x} y={p.y - 6} textAnchor="middle" fontSize="11" fontWeight="700"
                          fill={isLow ? "#6B7280" : "#0071e3"}>{t.heightM}m</text>
                        <text x={p.x} y={H + 13} textAnchor="middle" fontSize="11" fontWeight="700" fill="#1d1d1f">{t.timeStr}</text>
                        <text x={p.x} y={H + 25} textAnchor="middle" fontSize="10" fill="#9CA3AF">{isLow ? "저조" : "고조"}</text>
                      </g>
                    );
                  })}
                </svg>
              ...
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-lg bg-yellow-50 text-[13px] font-bold text-yellow-700"
                  >
                    지도 ↗
                  </button>
                </div>
        ...
        {seasonalNote && (
          <div className="rounded-xl bg-amber-50 px-4 py-3">
            <p className="text-[14px] text-amber-800 leading-relaxed">🌊 {seasonalNote}</p>
          </div>
        )}
      </section>
...
function SportTeamLogo({
  ...
  return (
    <div className="flex flex-col items-center gap-1" style={{ width: size + 20 }}>
      ...
      <p className="text-[12px] text-[#1d1d1f] text-center font-bold leading-tight"
...
function MatchCard({ m, assets, formatMatchDate }: {
  ...
  // 리그별 옅은 그라데이션 배경 (카드 본문)
  const cardBgGradient: Record<string, string> = {
    "K리그1": "linear-gradient(160deg,#ffffff 0%,#e8eefa 60%,#d9e1f5 100%)",
    "KBO":    "linear-gradient(160deg,#ffffff 0%,#fde7e9 60%,#f9d4d8 100%)",
    "V리그":  "linear-gradient(160deg,#ffffff 0%,#e3edfa 60%,#d2def0 100%)",
    "KBL":    "linear-gradient(160deg,#ffffff 0%,#efe5f5 60%,#dccaeb 100%)",
    "A매치":  "linear-gradient(160deg,#ffffff 0%,#fde2e2 60%,#f7c7c7 100%)",
  };
  const cardBg = cardBgGradient[meta.league] ?? "linear-gradient(160deg,#ffffff 0%,#f5f5f7 100%)";
...
function SportsSection() {
  ...
  // 지난 일정 숨김 (오늘 자정 이전 finished 제외) — "전체" 탭에서는 라이브 + 예정만
  const todayMidnight = new Date(); todayMidnight.setHours(0, 0, 0, 0);

  // 오늘 진행중 / 오늘 끝난 경기만 결과로 표시 (그 이전 finished 는 제외)
  const resultMatchesRaw = filtered
    .filter(m => {
      if (m.status === "live") return true;
      if (m.status !== "finished") return false;
      // 오늘 끝난 경기만 표시
      return new Date(m.match_date).getTime() >= todayMidnight.getTime();
    })
    .sort((a, b) => new Date(b.match_date).getTime() - new Date(a.match_date).getTime());

  const upcomingMatchesRaw = filtered
    .filter(m => m.status === "upcoming" && new Date(m.match_date).getTime() >= todayMidnight.getTime())
    .sort((a, b) => new Date(a.match_date).getTime() - new Date(b.match_date).getTime());

  // "전체" 탭에서는 종목별 라운드로빈으로 다양화
  function diversify(list: SportsMatch[], take: number): SportsMatch[] {
    if (filter !== "전체") return list.slice(0, take);
    const buckets = new Map<string, SportsMatch[]>();
    for (const m of list) {
      const arr = buckets.get(m.sport) ?? [];
      arr.push(m);
      buckets.set(m.sport, arr);
    }
  ...
  const resultMatches = diversify(resultMatchesRaw, 6);
  const upcomingMatches = diversify(upcomingMatchesRaw, 6);
...
// ─── 실거래가 위젯 ────────────────────────────────────────────
type AvgPyeongTier = "전체" | "20평대" | "30평대" | "40평대" | "50평대";
const AVG_PYEONG_TIERS: AvgPyeongTier[] = ["전체", "20평대", "30평대", "40평대", "50평대"];
function pyeongInTier(p: number, tier: AvgPyeongTier): boolean {
  if (tier === "전체") return true;
  if (tier === "20평대") return p >= 20 && p < 30;
  if (tier === "30평대") return p >= 30 && p < 40;
  if (tier === "40평대") return p >= 40 && p < 50;
  if (tier === "50평대") return p >= 50;
  return true;
}
...
function RealEstateWidget() {
  ...
  const [avgTier, setAvgTier] = useState<AvgPyeongTier>("전체");
  useEffect(() => {
  ...
  return (
    <section className="mx-4 mb-1 space-y-2">
      ...
      <button onClick={go}
        ...
        {myApt && mySz ? (
          <div className="px-4 py-4 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[12px] font-semibold text-white/80">내 집 시세</span>
              </div>
              <p className="text-[16px] font-bold text-white truncate">{myApt.name}</p>
              <p className="text-[12px] text-white/70">{myApt.dong} · {mySz.pyeong}평</p>
            </div>
            <div className="text-right shrink-0">
              <p className="text-[28px] font-black text-white leading-tight">{formatPrice(myCurr)}</p>
              {myDiff !== 0 && (
                <p className={`text-[13px] font-bold ${myDiff > 0 ? "text-red-200" : "text-blue-200"}`}>
          ...
          <div className="px-4 py-4 flex items-center gap-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[15px] font-bold text-white">내 집 등록</p>
              <p className="text-[12px] text-white/70">소식 → 시세 탭에서 내 집을 등록하세요</p>
            </div>
      ...
      {/* 검단 신도시 평균 실거래가 — 흰색 배경 + 평수 필터 */}
      <div className="w-full rounded-2xl bg-white border border-gray-100 overflow-hidden">
        <button onClick={go}
          className="w-full px-4 pt-3.5 pb-2 text-left active:bg-gray-50">
          <p className="text-[12px] text-gray-500 mb-0.5 font-medium">검단 신도시 평균 실거래가</p>
          <div className="flex items-end justify-between gap-2">
            <p className="text-[26px] font-black text-[#1d1d1f] leading-tight">
              {avgPrice > 0 ? formatPrice(avgPrice) : "—"}
            </p>
            {avgDiff !== 0 && (
              <p className={`text-[13px] font-bold pb-1 ${avgDiff > 0 ? "text-red-500" : "text-blue-500"}`}>
                {avgDiff > 0 ? "▲" : "▼"} {avgPct}%
              </p>
            )}
          </div>
          <p className="text-[11px] text-gray-400 mt-0.5">
            {avgTier === "전체" ? "전체 평수" : avgTier} · 최근 실거래 기준
          </p>
        </button>
        {/* 평수 티어 필터 */}
        <div className="flex gap-1.5 px-4 pb-3 pt-1 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {AVG_PYEONG_TIERS.map(t => (
            <button key={t}
              onClick={(e) => { e.stopPropagation(); setAvgTier(t); }}
...
/** "10:00 ~ 23:00" 형태 영업시간 → 현재 시각이 영업중인지 */
function isWithinHours(hoursStr: string | null, date: Date): boolean {
  if (!hoursStr) return false;
  if (/24시간|연중무휴/.test(hoursStr)) return true;
...
function getMartStatus(mart: Mart, date: Date): {
  ...
  if (day === 0) hours = mart.sunday_hours;
  else if (day === 6) hours = mart.saturday_hours;
  else hours = mart.weekday_hours;

  if (!hours) {
    return { isOpen: false, hours: null, reason: day === 0 ? "일요일 휴무" : "휴무" };
  }
  // 시간대 체크 (현재 시각 기준만; 미래/과거일 경우엔 시간대 체크 생략)
  const isToday = date.toDateString() === new Date().toDateString();
  if (!isToday) return { isOpen: true, hours };

  if (isWithinHours(hours, date)) return { isOpen: true, hours };
  return { isOpen: false, hours, reason: "영업 종료" };
}
...
function MartSection() {
  ...
  // 거리 계산 + 정렬 (영업중 우선 → 거리 오름차순)
  const martsWithDist = marts.map(m => {
    let distM: number | null = null;
    if (userLoc && m.lat != null && m.lng != null) {
      distM = haversineM(userLoc.lat, userLoc.lng, m.lat, m.lng);
    }
    const status = getMartStatus(m, now);
    return { mart: m, distM, status };
  });

  const sorted = [...martsWithDist].sort((a, b) => {
    ...
    return a.mart.sort_order - b.mart.sort_order;
  });
  ...
  const visible = expanded ? sorted : sorted.slice(0, DEFAULT_MART_COUNT);
  const hasMore = sorted.length > DEFAULT_MART_COUNT;
    ...
    <>
      ...
      <div className="bg-white rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-[#f5f5f7]">
          {visible.map(({ mart, distM, status: todayStatus }) => {
            const tmrStatus   = showTomorrow ? getMartStatus(mart, tomorrow) : null;
            const distLabel = distM != null ? formatDistance(distM) : mart.distance;
            ...
            return (
              <div key={mart.id} className="px-4 py-3.5 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={`text-[12px] font-semibold ${todayStatus.isOpen ? "text-[#059669]" : "text-[#F04452]"}`}>
                      {todayStatus.isOpen
                        ? `영업 중 · ${todayStatus.hours}`
                        : todayStatus.hours
                          ? `${todayStatus.reason ?? "영업 종료"} · ${todayStatus.hours}`
                          : `오늘 휴무${todayStatus.reason ? ` (${todayStatus.reason})` : ""}`
                      }
                    </span>
                  </div>
                ...
                <div className="flex flex-col items-end gap-1.5 shrink-0">
                  {distLabel && <span className="text-[12px] text-[#6e6e73]">{distLabel}</span>}
        ...
        {hasMore && (
          <button
            ...
            className="w-full py-3 border-t border-[#f5f5f7] flex items-center justify-center gap-1 text-[13px] font-semibold text-[#0071e3] active:bg-[#f5f5f7]">
            {expanded
              ? <>접기 <ChevronUp size={14} /></>
              : <>더보기 ({sorted.length - DEFAULT_MART_COUNT}곳 더) <ChevronDown size={14} /></>
            }
          </button>
        )}
      </div>
  ...
  return (
    <div className="flex items-center gap-2.5 px-3 py-2 animate-slide-up"
      ...
      <div className={`shrink-0 rounded-xl px-2.5 py-1.5 min-w-[52px] text-center ${
        ...
        {arriving ? (
          <span className="text-[#F04452] text-[11px] font-black animate-led-blink">곧 도착</span>
        ) : (
          <>
