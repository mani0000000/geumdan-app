...
// ─── 주유소 바텀시트 (상가지도용) ───────────────────────────
function GasStationSheet({
  station, onClose,
}: {
  station: BasicGasStation;
  onClose: () => void;
}) {
  return (
      ...
      >
        ...
        <div className="px-5 pt-4 pb-6">
          <div className="flex items-start gap-3 mb-4">
            <div
              className="w-14 h-14 rounded-2xl flex items-center justify-center text-center shrink-0"
            ...
            <button onClick={onClose} className="active:opacity-60 shrink-0 mt-1">
          ...
          <a
            href="/gas"
            className="w-full h-12 bg-[#FF6B35] rounded-2xl flex items-center justify-center gap-2 text-white text-[15px] font-bold active:opacity-80"
          >
...
export default function StoresPage() {
  ...
  const [gasStations,    setGasStations]    = useState<BasicGasStation[]>([]);
  const [selectedGasId,  setSelectedGasId]  = useState<string | null>(null);
  const [gasDetailSheet, setGasDetailSheet] = useState<BasicGasStation | null>(null);
