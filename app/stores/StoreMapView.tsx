...
import type { BasicGasStation } from "@/lib/db/gas-stations";
...
// ── 주유소 마커 아이콘 ────────────────────────────────────────
function gasStationIcon(s: BasicGasStation, selected: boolean): L.DivIcon {
  const bg     = selected ? s.brandColor : "white";
  const fg     = selected ? "white" : s.brandColor;
  const border = selected ? s.brandColor : "#d2d2d7";
  return L.divIcon({
    className:   "",
    iconSize:    [52, 44] as [number, number],
    iconAnchor:  [26, 44] as [number, number],
    popupAnchor: [0, -46] as [number, number],
    html: `
<div style="display:flex;flex-direction:column;align-items:center;cursor:pointer">
  <div style="background:${bg};border:2px solid ${border};border-radius:10px;padding:3px 7px;box-shadow:0 2px 8px rgba(0,0,0,.18);text-align:center;min-width:44px">
    <div style="font-size:14px;line-height:1">⛽</div>
    <div style="font-size:9px;font-weight:800;color:${fg};white-space:nowrap;margin-top:1px">${s.brandShort}</div>
  </div>
  <div style="width:2px;height:6px;background:${border}"></div>
</div>`,
  });
}
...
interface Props {
  ...
  gasStations?:    BasicGasStation[];
  selectedGasId?:  string | null;
  onGasSelect?:    (s: BasicGasStation) => void;
}

// ── 메인 컴포넌트 ─────────────────────────────────────────────
export default function StoreMapView({ buildings, selectedId, onSelect, dimmedIds, gasStations, selectedGasId, onGasSelect }: Props) {
      ...
      >
        ...
        {gasStations?.map(s => (
          <Marker
            ...
            icon={gasStationIcon(s, selectedGasId === s.id)}
            eventHandlers={{ click: () => onGasSelect?.(s) }}
          />
        ))}
