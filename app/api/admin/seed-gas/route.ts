...
import { katecToWgs84, SCAN_CENTERS, wgs84ToKatec } from "@/lib/api/opinet";
...
async function fetchOpinet(lat: number, lng: number, prodcd: string, apiKey: string): Promise<OpinetStation[]> {
  const { x, y } = wgs84ToKatec(lat, lng);
  const url =
    `https://www.opinet.co.kr/api/aroundAll.do?code=${apiKey}` +
    `&x=${x}&y=${y}&radius=${RADIUS_M}` +
...
export async function POST(req: NextRequest) {
  ...
  // 1. 오피넷 - 여러 중심점에서 3개 유종 호출 (검단신도시 신·구 + 김포 커버)
