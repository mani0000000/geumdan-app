...
export default function SignupPage() {
  ...
  const CheckRow = ({ k, label }: { k: keyof typeof agree; label: string }) => (
    <button onClick={() => toggle(k)} className="w-full flex items-center gap-3 py-3 active:opacity-70">
      ...
      <span className="text-[15px] text-[#191F28]">{label}</span>
    </button>
  );
  ...
  return (
    <div className="min-h-dvh bg-white flex flex-col">
      ...
      <div className="px-6 pb-0">
        ...
        <p className="text-[13px] text-[#8B95A1] mb-1">{step + 1} / {steps.length}</p>
        <h2 className="text-[23px] font-bold text-[#191F28] leading-snug mb-6">
          {step === 0 && "약관에 동의해 주세요"}
          {step === 1 && "본인 인증을 해 주세요"}
          {step === 2 && "검단 어디에 사세요?"}
        </h2>
        ...
        {step === 0 && (
          <div className="flex flex-col gap-1">
            <button onClick={toggleAll} className="flex items-center gap-3 p-4 rounded-2xl bg-[#F2F4F6] active:opacity-70">
              ...
              <span className="text-[16px] font-bold text-[#191F28]">전체 동의</span>
            </button>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex gap-2">
              <input value={phone} onChange={e => setPhone(e.target.value)} type="tel" placeholder="휴대폰 번호"
                className="flex-1 h-[52px] px-4 rounded-xl bg-[#F2F4F6] text-[16px] outline-none focus:ring-2 focus:ring-[#3182F6]" />
            ...
            {codeSent && (
              <>
                <input value={code} onChange={e => setCode(e.target.value)} type="text" placeholder="인증번호 6자리" maxLength={6}
                  className="w-full h-[52px] px-4 rounded-xl bg-[#F2F4F6] text-[16px] outline-none focus:ring-2 focus:ring-[#3182F6]" />
        ...
        {step === 2 && (
          <div className="flex flex-col gap-4">
            <input value={nickname} onChange={e => setNickname(e.target.value)} placeholder="닉네임 (2~12자)"
              className="w-full h-[52px] px-4 rounded-xl bg-[#F2F4F6] text-[16px] outline-none focus:ring-2 focus:ring-[#3182F6]" />
            <div>
              <p className="text-[14px] text-[#8B95A1] mb-2">거주 동네</p>
              <div className="grid grid-cols-3 gap-2">
                {dongs.map(d => (
                  <button key={d} onClick={() => setDong(d)}
                    className={`h-10 rounded-xl text-[14px] font-medium transition-colors active:opacity-70 ${dong === d ? "bg-[#3182F6] text-white" : "bg-[#F2F4F6] text-[#191F28]"}`}>
                    {d}
                  </button>
                ))}
              </div>
