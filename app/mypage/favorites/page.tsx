    ...
    <div className="min-h-dvh bg-[#f5f5f7]">
      ...
      <div className="px-4 pt-4 pb-10">
        {loading ? (
          ...
          buses.length === 0 ? (
            <Empty
              icon={<Bus size={28} className="text-[#86868b]" />}
              title="즐겨찾는 노선이 없어요"
              sub="교통 페이지에서 자주 타는 버스를 추가해 보세요"
              cta="교통 가기" href="/transport/"
            />
          ) : (
          ...
          stores.length === 0 ? (
            <Empty
              icon={<Store size={28} className="text-[#86868b]" />}
              title="찜한 상가가 없어요"
              sub="상가 페이지에서 자주 가는 매장을 추가해 보세요"
              cta="상가 가기" href="/stores/"
            />
          ) : (
            <ul className="space-y-2.5">
              {stores.map(s => (
          ...
          apts.length === 0 ? (
            <Empty
              icon={<Building size={28} className="text-[#86868b]" />}
              title="관심 아파트가 없어요"
              sub="시세에서 단지를 추가해 보세요"
              cta="시세 가기" href="/community/?tab=시세"
            />
          ) : (
            <ul className="space-y-2.5">
              {apts.map(a => (
...
}: { icon: React.ReactNode; title: string; sub: string; cta: string; href: string }) {
  return (
