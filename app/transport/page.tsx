    ...
    try {
      const results = await Promise.all(
        nearbyStops.map(async (stop) => {
          ...
          if (arrivals.length === 0) return [stop.id, stop.routes] as const;
        ...
        <div className="px-4 space-y-3">
                ...
                <div key={stop.id} className="bg-white rounded-2xl overflow-hidden">
                  <button onClick={() => setExpanded(open ? null : stop.id)}
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={e => {
                        e.stopPropagation();
                        setFavs(p => { const n = new Set(p); n.has(stop.id) ? n.delete(stop.id) : n.add(stop.id); return n; });
                      }}>
          ...
          {!isLive && (
            <div className="bg-[#FFFDE7] rounded-2xl px-4 py-3">
              <p className="text-[12px] font-bold text-[#F57F17]">💡 실시간 버스 연동하기</p>
              <p className="text-[11px] text-[#F57F17]/80 mt-1 leading-relaxed">
                공공데이터포털에서 인천버스 API 키를 발급받아<br />
                <code className="font-mono bg-[#FFF9C4] px-1 rounded">NEXT_PUBLIC_BUS_API_KEY</code> 환경변수를 설정하면 실시간 정보가 표시됩니다.
              </p>
            </div>
          )}
        </div>
