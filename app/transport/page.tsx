...
function ArrivalBadge({ min, live }: { min: number; live: boolean }) {
  if (!live || min === -1) {
    return (
      <div className="bg-[#e5e5ea] rounded-xl px-3 py-2 text-center min-w-[56px]">
        <span className="text-[#86868b] text-[13px] font-bold leading-none">--</span>
      </div>
    );
  }
  const bg = min <= 3 ? "bg-[#F04452]" : min <= 7 ? "bg-[#FF9500]" : "bg-[#0071e3]";
  return (
    <div className={`${bg} rounded-xl px-3 py-2 text-center min-w-[56px]`}>
      {min <= 0
        ? <span className="text-white text-[12px] font-black tracking-tight">곧도착</span>
        : <>
            <span className="text-white text-[20px] font-black leading-none tracking-tight">{min}</span>
            <span className="text-white/75 text-[10px] font-semibold block leading-none mt-0.5">분 후</span>
          </>
      }
    </div>
  );
        ...
        <div className="px-4 space-y-3">
          ...
          <div className="bg-white rounded-2xl flex items-center gap-2.5 px-4 h-11 shadow-sm border border-gray-100">
            ...
            <input
              ...
              className="flex-1 text-[14px] outline-none bg-transparent placeholder:text-[#c7c7cc] text-[#1d1d1f]"
            />
            {busSearch && (
              <button onClick={() => setBusSearch("")} className="text-[#86868b] active:opacity-60 shrink-0">
                <X size={15} />
              </button>
            )}
          </div>
          ...
          {loading ? (
            ...
            filteredStops.map((stop, idx) => {
              ...
              return (
                <div key={stop.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                  {/* 정류장 헤더 */}
                  <button onClick={() => setExpanded(open ? null : stop.id)}
                    className="w-full flex items-center justify-between px-4 py-4 active:bg-[#f5f5f7] transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="relative shrink-0">
                        ...
                        {idx === 0 && (
                          <span className="absolute -top-1 -right-1 text-[9px] font-black bg-[#0071e3] text-white px-1.5 rounded-full leading-tight py-0.5">
                            최근접
                          </span>
                        )}
                      </div>
                      <div className="text-left min-w-0">
                        <p className="text-[15px] font-bold text-[#1d1d1f] truncate">{stop.name}</p>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <Navigation size={10} className="text-[#0071e3] shrink-0" />
                  ...
                  <div className="px-3 pb-3 space-y-2">
                    {stop.loadingArrivals ? (
                      [1, 2].map(i => (
                        <div key={i} className="flex items-center justify-between bg-[#f5f5f7] rounded-xl px-3 py-3.5 animate-pulse">
                          <div className="flex items-center gap-2.5">
                            <div className="w-12 h-9 bg-[#d2d2d7] rounded-lg" />
                            <div className="space-y-2">
                              <div className="h-3.5 w-32 bg-[#d2d2d7] rounded" />
                              <div className="h-3 w-20 bg-[#d2d2d7] rounded" />
                            </div>
                          </div>
                      ...
                      <div className="bg-[#f5f5f7] rounded-xl px-3 py-4 text-center">
                        <p className="text-[13px] text-[#86868b]">운행 정보 없음</p>
                      </div>
                    ) : (
                      <>
                        {displayArrivals.map((a, i) => (
                          <div key={i}
                            className="flex items-center justify-between bg-[#f5f5f7] rounded-xl px-3 py-3 cursor-pointer active:bg-[#e5e5ea] transition-colors"
                            onClick={() => setSelectedArrival(a)}>
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              {/* 노선 번호 뱃지 */}
                              <div className={`rounded-lg px-2.5 py-1.5 min-w-[48px] text-center shrink-0 ${a.isScheduled ? "bg-[#8e8e93]" : "bg-[#0071e3]"}`}>
                                <span className="text-white text-[14px] font-black leading-none">{a.routeNo}</span>
                                {a.isExpress && <p className="text-yellow-200 text-[9px] font-bold leading-none mt-0.5">급행</p>}
                              </div>
                              {/* 목적지 + 서브 정보 */}
                              <div className="min-w-0">
                                ...
                                <p className="text-[12px] text-[#6e6e73] mt-0.5 truncate">
                                  {a.isScheduled ? "경유 노선 · 탭하면 전 경로 보기" : a.remainingStops > 0 ? `${a.remainingStops}정류장 전` : "곧 도착 예정"}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0 ml-2">
                              {!a.isScheduled && (
                                <button onClick={e => { e.stopPropagation(); toggleRoute(routeFavKey(stop.id, a)); }}
                                  className="p-1.5 active:opacity-60 rounded-full">
                        ...
                        {!open && stop.arrivals.length > 3 && (
                          <button onClick={() => setExpanded(stop.id)}
                            className="w-full text-[13px] font-semibold text-[#0071e3] text-center py-2 active:opacity-60">
          ...
          {/* 범례 — 실시간 연동 시에만 */}
          {isLive && (
            <div className="bg-white rounded-2xl px-4 py-3 flex gap-5 shadow-sm border border-gray-100">
              {[["bg-[#F04452]", "3분 이내"], ["bg-[#FF9500]", "7분 이내"], ["bg-[#0071e3]", "8분 이상"]].map(([c, l]) => (
                <div key={l} className="flex items-center gap-1.5">
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${c}`} />
                  <span className="text-[12px] font-medium text-[#6e6e73]">{l}</span>
                </div>
              ))}
            </div>
          )}
                ...
                <div key={st.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-center gap-2.5 px-4 py-3" style={{ background: st.lineColor }}>
                    <button onClick={() => setTimetableStation(st)}
                      className="flex items-center gap-2.5 flex-1 min-w-0 active:opacity-75 text-left">
                      <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center shrink-0">
                        <span className="text-[13px] font-black leading-none" style={{ color: st.lineColor }}>{lineShort}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-white font-extrabold text-[16px] block truncate leading-tight">{st.displayName}</span>
                        <span className="text-white/70 text-[11px] font-medium">{st.line}</span>
                      </div>
                      <span className="flex items-center gap-0.5 bg-white/20 rounded-lg px-2 py-1 text-white text-[11px] font-semibold shrink-0">
                        <Clock size={10} className="shrink-0" />시간표
                      </span>
                    </button>
                    {!st.planned && (
                      <button onClick={() => toggleSubway(st.id)} className="p-1.5 active:opacity-60 shrink-0 rounded-full">
                        <Star size={18}
                  ...
                  {st.loadingArrivals ? (
                    ...
                    <p className="px-4 py-4 text-[13px] text-[#86868b] text-center">운행 종료</p>
                  ) : (
                    <div className="grid grid-cols-2 divide-x divide-[#f5f5f7]">
                      {[
                        ...
                        <div key={col} className="px-3.5 py-3">
                          {/* 방면 헤더 */}
                          <div className="flex items-center gap-1 mb-3">
                            <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: st.lineColor }} />
                            <span className="text-[12px] font-bold text-[#1d1d1f] flex-1 truncate">{label} 방면</span>
                          </div>
                          {dirArrivals.length > 0 ? dirArrivals.map((a, i) => (
                            <div key={i} className={`flex items-start justify-between gap-1 ${i > 0 ? "mt-2.5 pt-2.5 border-t border-[#f5f5f7]" : ""}`}>
                              <div className="flex-1 min-w-0">
                                <span className="text-[13px] font-semibold text-[#1d1d1f] block truncate leading-tight">{a.terminalStation}</span>
                                {!isEstimated && a.currentStation && (
                                  <span className="text-[10px] text-[#86868b] mt-0.5 block">{a.currentStation} 출발</span>
                                )}
                                {a.isExpress && (
                                  <span className="text-[10px] text-[#E65100] font-bold mt-0.5 block">{a.trainTypeName ?? "급행"}</span>
                                )}
                              </div>
                              <div className="text-right shrink-0">
                                <span className={`text-[18px] font-black leading-none block ${
                                  a.arrivalMin <= 2 ? "text-[#F04452]" : a.arrivalMin <= 7 ? "text-[#FF9500]" : "text-[#0071e3]"
                                }`}>
                                  {a.arrivalMin <= 0 ? "곧" : a.arrivalMin}
                                </span>
                                {a.arrivalMin > 0 && (
                                  <span className="text-[10px] text-[#86868b] leading-none">분 후</span>
                                )}
                              </div>
                            </div>
                          )) : (
                            <p className="text-[12px] text-[#86868b]">정보 없음</p>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* 첫차/막차 */}
                  {st.timetable.upFirst !== "-" && (
                    <div className="flex gap-2 px-3 pb-3 pt-2 border-t border-[#f5f5f7]">
                      <div className="flex-1 bg-[#f5f5f7] rounded-xl px-3 py-2.5">
                        <p className="text-[10px] text-[#86868b] font-medium mb-1">{st.timetable.upDirection} 첫/막차</p>
                        <p className="text-[12px] font-bold text-[#1d1d1f]">{st.timetable.upFirst} / {st.timetable.upLast}</p>
                      </div>
                      <div className="flex-1 bg-[#f5f5f7] rounded-xl px-3 py-2.5">
                        <p className="text-[10px] text-[#86868b] font-medium mb-1">{st.timetable.downDirection} 첫/막차</p>
                        <p className="text-[12px] font-bold text-[#1d1d1f]">{st.timetable.downFirst} / {st.timetable.downLast}</p>
                      </div>
                    </div>
                  )}
