  ...
  return (
    <div className="min-h-dvh bg-[#F2F4F6]">
      ...
      <div className="flex items-center gap-2 px-4 h-14 border-b border-[#F2F4F6] bg-white sticky top-0 z-10">
        ...
        <h1 className="text-[17px] font-bold text-[#191F28] truncate">{apt.name}</h1>
      </div>
      ...
      <div className="pb-8 space-y-3">
        ...
        <div className="bg-white px-5 py-5">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h2 className="text-[20px] font-black text-[#191F28]">{apt.name}</h2>
            ...
            <div className="text-right">
              <p className="text-[22px] font-black text-[#00C471]">{formatPrice(apt.recentDeal?.price ?? 0)}</p>
              <p className="text-[11px] text-[#8B95A1]">최근 실거래가</p>
            </div>
          </div>
        ...
        <div className="bg-white px-5 py-4">
          <p className="text-[14px] font-bold text-[#191F28] mb-3">평형별 평균 시세</p>
            ...
            {apt.sizes.map((s, i) => {
              ...
              return (
                <div key={i} className="bg-[#F2F4F6] rounded-2xl p-4">
                  <p className="text-[12px] text-[#8B95A1] mb-1">{s.pyeong}평 ({s.sqm}㎡)</p>
                  <p className="text-[20px] font-black text-[#191F28]">{formatPrice(s.avgPrice)}</p>
                  <div className="flex items-center gap-1 mt-1">
                    {diff > 0
                      ? <><TrendingUp size={11} className="text-[#F04452]" /><span className="text-[11px] font-bold text-[#F04452]">▲ {formatPrice(diff)}</span></>
                      : diff < 0
                      ? <><TrendingDown size={11} className="text-[#3182F6]" /><span className="text-[11px] font-bold text-[#3182F6]">▼ {formatPrice(Math.abs(diff))}</span></>
                      : <span className="text-[11px] text-[#8B95A1]">보합</span>
                    }
                  </div>
                </div>
              );
        ...
        {/* Price chart */}
        <div className="bg-white px-5 py-4">
          <p className="text-[14px] font-bold text-[#191F28] mb-1">{sz0.pyeong}평 시세 추이</p>
          <p className="text-[12px] text-[#8B95A1] mb-4">최근 2년 평균 매매가</p>
          ...
          {sz1 && (
            <>
              <p className="text-[14px] font-bold text-[#191F28] mt-5 mb-1">{sz1.pyeong}평 시세 추이</p>
              <ResponsiveContainer width="100%" height={160}>
        ...
        {/* Recent transactions */}
        <div className="bg-white px-5 py-4">
          <p className="text-[14px] font-bold text-[#191F28] mb-3">최근 실거래 내역</p>
          <div className="space-y-0">
            {mockTransactions.map((t, i) => (
              <div key={i} className={`flex items-center py-3.5 ${i !== mockTransactions.length - 1 ? "border-b border-[#F2F4F6]" : ""}`}>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${t.type === "매매" ? "bg-[#D1FAE5] text-[#065F46]" : "bg-[#EBF3FE] text-[#1E40AF]"}`}>
                      {t.type}
                    </span>
                    <span className="text-[13px] text-[#8B95A1]">{t.pyeong}평 · {t.floor}층</span>
                  </div>
                  <p className="text-[12px] text-[#B0B8C1] mt-0.5">{t.date}</p>
                </div>
                <p className="text-[16px] font-black text-[#191F28]">{formatPrice(t.price)}</p>
              </div>
            ))}
          </div>
          <p className="text-[11px] text-[#B0B8C1] mt-3">※ 국토교통부 실거래가 공개시스템 기준</p>
        </div>

        {/* Nearby info */}
        <div className="bg-white px-5 py-4">
          <p className="text-[14px] font-bold text-[#191F28] mb-3">주변 편의시설</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={label} className="bg-[#F2F4F6] rounded-xl p-3 text-center">
                ...
                <p className="text-[12px] font-bold text-[#191F28] mt-0.5">{value}</p>
              </div>
            ))}
          </div>
        </div>
