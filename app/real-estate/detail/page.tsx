...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div className="bg-[#1d1d1f] rounded-xl px-3 py-2">
      <p className="text-[#6e6e73] text-[12px]">{label}</p>
  ...
  return (
    <div className="min-h-dvh bg-[#f5f5f7]">
      ...
      <div className="flex items-center gap-2 px-4 h-14 border-b border-[#f5f5f7] bg-white sticky top-0 z-10">
        ...
        <h1 className="text-[18px] font-bold text-[#1d1d1f] truncate">{apt.name}</h1>
      </div>
      ...
      <div className="pb-8 space-y-3">
        ...
        <div className="bg-white px-5 py-5">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h2 className="text-[21px] font-black text-[#1d1d1f]">{apt.name}</h2>
              <div className="flex items-center gap-1.5 mt-1">
                <MapPin size={12} className="text-[#86868b]" />
                <span className="text-[14px] text-[#6e6e73]">{apt.dong}</span>
              </div>
            </div>
            <div className="text-right">
              <p className="text-[23px] font-black text-[#00C471]">{formatPrice(apt.recentDeal?.price ?? 0)}</p>
              <p className="text-[12px] text-[#6e6e73]">최근 실거래가</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {[
              ...
              <div key={label} className="flex items-center gap-1.5 bg-[#f5f5f7] rounded-xl px-3 py-1.5">
                <Icon size={12} className="text-[#6e6e73]" />
                <span className="text-[13px] text-[#424245] font-medium">{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white px-5 py-4">
          <p className="text-[15px] font-bold text-[#1d1d1f] mb-3">평형별 평균 시세</p>
            ...
            {apt.sizes.map((s, i) => {
              ...
              return (
                <div key={i} className="bg-[#f5f5f7] rounded-2xl p-4">
                  <p className="text-[13px] text-[#6e6e73] mb-1">{s.pyeong}평 ({s.sqm}㎡)</p>
                  <p className="text-[21px] font-black text-[#1d1d1f]">{formatPrice(s.avgPrice)}</p>
                  <div className="flex items-center gap-1 mt-1">
                    {diff > 0
                      ? <><TrendingUp size={11} className="text-[#F04452]" /><span className="text-[12px] font-bold text-[#F04452]">▲ {formatPrice(diff)}</span></>
                      : diff < 0
                      ? <><TrendingDown size={11} className="text-[#0071e3]" /><span className="text-[12px] font-bold text-[#0071e3]">▼ {formatPrice(Math.abs(diff))}</span></>
                      : <span className="text-[12px] text-[#6e6e73]">보합</span>
                    }
                  </div>
                </div>
              );
        ...
        {/* Price chart — 24평 */}
        <div className="bg-white px-5 py-4">
          <p className="text-[15px] font-bold text-[#1d1d1f] mb-1">{sz0.pyeong}평 시세 추이</p>
          <p className="text-[13px] text-[#6e6e73] mb-4">최근 15개월 평균 매매가 (국토교통부)</p>
          <ResponsiveContainer width="100%" height={160}>
            <LineChart data={sz0.priceHistory} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f5f5f7" />
              <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#86868b" }} tickFormatter={v => v.slice(2)} />
              <YAxis tick={{ fontSize: 10, fill: "#86868b" }} tickFormatter={v => `${(v / 10000).toFixed(0)}억`} />
          ...
          {sz1 && (
            <>
              <p className="text-[15px] font-bold text-[#1d1d1f] mt-5 mb-1">{sz1.pyeong}평 시세 추이</p>
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={sz1.priceHistory} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: "#86868b" }} tickFormatter={v => `${(v / 10000).toFixed(0)}억`} />
                  <Tooltip content={<ChartTooltip />} />
                  <Line type="monotone" dataKey="price" stroke="#0071e3" strokeWidth={2.5} dot={false} activeDot={{ r: 4, fill: "#0071e3" }} />
                </LineChart>
              </ResponsiveContainer>
            </>
          )}
        ...
        {/* Recent transactions */}
        <div className="bg-white px-5 py-4">
          <p className="text-[15px] font-bold text-[#1d1d1f] mb-3">최근 실거래 내역</p>
          <div className="space-y-0">
            {recentTx.map((t, i) => (
              <div key={i} className={`flex items-center py-3.5 ${i !== recentTx.length - 1 ? "border-b border-[#f5f5f7]" : ""}`}>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[12px] font-bold px-2 py-0.5 rounded-full ${t.type === "매매" ? "bg-[#D1FAE5] text-[#065F46]" : "bg-[#e8f1fd] text-[#1E40AF]"}`}>
                      {t.type}
                    </span>
                    <span className="text-[14px] text-[#6e6e73]">{t.pyeong}평 · {t.floor}층</span>
                  </div>
                  <p className="text-[13px] text-[#86868b] mt-0.5">{t.date}</p>
                </div>
                <p className="text-[17px] font-black text-[#1d1d1f]">{formatPrice(t.price)}</p>
              </div>
            ))}
          </div>
          <p className="text-[12px] text-[#86868b] mt-3">※ 국토교통부 실거래가 공개시스템 기준</p>
        </div>

        {/* Nearby info */}
        <div className="bg-white px-5 py-4">
          <p className="text-[15px] font-bold text-[#1d1d1f] mb-3">주변 편의시설</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={label} className="bg-[#f5f5f7] rounded-xl p-3 text-center">
                ...
                <p className="text-[13px] font-bold text-[#1d1d1f] mt-0.5">{value}</p>
              </div>
            ))}
          </div>
        </div>
